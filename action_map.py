...
def vr_actionset_active_update(context):
    ...
    if scene.mocapvr_actions_use_gamepad and session_state.actionmaps.find(
        session_state, defaults.VRDefaultActionmaps.GAMEPAD.value
    ):
        session_state.active_action_set_set(
            context, defaults.VRDefaultActionmaps.GAMEPAD.value
        )
    else:
...
def vr_create_actions(context: bpy.context):
    ...
    scene = context.scene
    if not scene.mocapvr_actions_enable:
        return
    ...
    for am in session_state.actionmaps:
        ...
        if controller_grip_name and controller_aim_name:
            session_state.controller_pose_actions_set(
                context, am.name, controller_grip_name, controller_aim_name
            )
...
def vr_load_actionmaps(session_state, filepath):
    ...
    action_map_io.actionconfig_init_from_data(
        session_state, file.actionconfig_data, file.actionconfig_version
    )
...
def register():
    bpy.types.Scene.mocapvr_actions_enable = bpy.props.BoolProperty(
        name="Use Controller Actions",
        description="Enable default VR controller actions, including controller poses and haptics",
        default=True,
    )
    bpy.types.Scene.mocapvr_actions_use_gamepad = bpy.props.BoolProperty(
    ...
    bpy.types.Scene.mocapvr_actions_enable_vive_focus = bpy.props.BoolProperty(
...
def unregister():
    del bpy.types.Scene.mocapvr_actions_enable
    del bpy.types.Scene.mocapvr_actions_use_gamepad
    del bpy.types.Scene.mocapvr_actions_enable_huawei
    del bpy.types.Scene.mocapvr_actions_enable_reverb_g2
    del bpy.types.Scene.mocapvr_actions_enable_vive_cosmos
    del bpy.types.Scene.mocapvr_actions_enable_vive_focus
