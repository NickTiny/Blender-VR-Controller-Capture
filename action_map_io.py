...
def ami_data_from_args(ami, args):
    ...
    if ami.type == 'FLOAT' or ami.type == 'VECTOR2D':
        ...
        ami.haptic_match_user_paths = (
            True if (args["haptic_match_user_paths"] == 'True') else False
        )
        ...
        ami.pose_is_controller_grip = (
            True if (args["pose_is_controller_grip"] == 'True') else False
        )
        ami.pose_is_controller_aim = (
            True if (args["pose_is_controller_aim"] == 'True') else False
        )
...
def amb_args_as_data(amb, type):
    ...
    if type == 'FLOAT' or type == 'VECTOR2D':
        ...
        s.append(
            f"\"pose_location\": '{amb.pose_location.x, amb.pose_location.y, amb.pose_location.z}'"
        )
        s.append(
            f"\"pose_rotation\": '{amb.pose_rotation.x, amb.pose_rotation.y, amb.pose_rotation.z}'"
        )
...
def actionconfig_import_from_data(
    session_state, actionconfig_data, *, actionconfig_version=(0, 0, 0)
):
    # Load data in the format defined above.
    #
    # Runs at load time, keep this fast!
